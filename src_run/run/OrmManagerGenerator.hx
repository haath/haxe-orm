...
class OrmManagerGenerator
{
	...
	function getAutogenManager(db:Db, table:String, vars:Array<OrmHaxeVar>, modelClassName:String, autogenManagerClassName:String, customOrmClassName:String, positions:OrmPositions) : HaxeClass
	{
		...
		(
			...
			+ vars.map(w -> '_obj.' + w.haxeName + ' = ' + w.haxeName + ';').join('\n') + "\n"
			+ "return _obj;",
			true
		);
		...
		(
			...
			modelClassName,
			  "var _obj = new " + modelClassName + "(db, orm);\n"
			+ vars.map(w -> '_obj.' + w.haxeName + " = Reflect.field(d, '" + w.haxeName + "');").join('\n') + "\n"
			+ "return _obj;"
			, true
		);
		...
		var getVars = vars.filter(w -> w.isKey);
		if (getVars.length > 0)
		{
			model.addMethod
			(
		...
		var createVars = vars.filter(w -> !w.isAutoInc);
		...
		(
			...
			createVars,
			modelClassName,
			createVars.filter(positions.is).map(w ->
				  "if (" + w.haxeName + " == null)\n"
				+ "{\n"
				+ "\tposition = db.query('SELECT MAX(`" + w.name + "`) FROM `" + table + "`"
					+ getWhereSql(getForeignKeyVars(db, table, vars))
					+ ").getIntResult(0) + 1;\n"
				+ "}\n\n"
			).join("")
			+"db.query('INSERT INTO `" + table + "`("
				+ createVars.map(w -> "`" + w.name + "`").join(", ")
			+") VALUES (' + "
				+ createVars.map(w -> "db.quote(" + w.haxeName + ")").join(" + ', ' + ")
			+" + ')');\n"
			+"return newModelFromParams(" + vars.map(w -> w.isAutoInc ? 'db.lastInsertId()' : w.haxeName).join(", ") + ");"
		);

		model.addMethod
		(
			'createNamed',
			[ OrmTools.createVar("data", "{ " + createVars.map(w -> w.haxeName + ":" + w.haxeType).join(", ") + " }") ],
			modelClassName,
			createVars.filter(w -> positions.is(w)).map(w ->
				"if (data." + w.haxeName + " == null)\n"
				+ "{\n"
				+ "\tdata." + w.haxeName + " = db.query('SELECT MAX(`" + w.name + "`) FROM `" + table + "`"
					+ getWhereSql(getForeignKeyVars(db, table, vars))
					+ ").getIntResult(0) + 1;\n"
				+ "}\n\n"
			).join("")
			+"db.query('INSERT INTO `" + table + "`("
				+ createVars.map(w -> "`" + w.name + "`").join(", ")
			+") VALUES (' + "
				+ createVars.map(w -> "db.quote(data." + w.haxeName + ")").join(" + ', ' + ")
			+" + ')');\n"
			+"return newModelFromParams(" + vars.map(w -> w.isAutoInc ? 'db.lastInsertId()' : "data." + w.haxeName).join(", ") + ");"
		);

		var dataVars = [ OrmTools.createVar("data", "{ " + createVars.map(w -> (w.isKey ? "" : "?") + w.haxeName + ":" + w.haxeType).join(", ") + " }") ];

		if (vars.exists(w -> w.isKey))
		{
			model.addMethod
			(
				'createOptional',
				...
				+"return get(" + getVars.map(w -> w.isAutoInc ? 'db.lastInsertId()' : "data." + w.haxeName).join(", ") + ");"
			);
		}
		...
		(
			...
			+createVars.map(w ->
				w.isKey
					? "fields.push('`" + w.name + "`'); values.push(db.quote(data." + w.haxeName + "));\n"
					: "if (Reflect.hasField(data, '" + w.haxeName + "')) { fields.push('`" + w.name + "`'); values.push(db.quote(data." + w.haxeName + ")); }\n"
			).join("")
			+"db.query('INSERT INTO `" + table + "`(' + fields.join(\", \") + ') VALUES (' + values.join(\", \") + ')');\n"
		);

		var deleteVars = vars.filter(w -> w.isKey);
		...
		{
            var vs = vars.filter(w -> fields.has(w.name));
	...
	function createGetByMethodOne(table:String, vars:Array<OrmHaxeVar>, modelClassName:String, whereVars:Array<OrmHaxeVar>, model:HaxeClass) : Void
	{
        ...
        model.addMethod
		(
			'getBy' + whereVars.map(w -> w.haxeName.capitalize()).join('And'),
			whereVars,
			modelClassName,
	...
	{
		var positionVar = vars.filter(w -> positions.is(w));
		return positionVar.length == 0 ? "null" : "'" + positionVar.map(w -> w.name).join(", ") + "'";
	}
